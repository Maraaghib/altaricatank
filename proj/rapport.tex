...
\paragraph{Interprétation des résultats}
\paragraph{Interprétation des résultats : Contrôleur sans débit optimisé}
Ce controlleur évite les bloquages, car on a la variable $deadlock = 0$. cependant, il n'est pas possible d'éviter les Niveaux Critiques, qui sont au nombre de 413.
On peut aussi voir que le contrôleur a 17 transitions partant de son état initial.
...
\paragraph{Interprétation des résultats : }
Ce contrôleur optimisé permet de toujours faire son travail en évitant tout blocages, étant donné que $deadlock = 0$ et $NC = 0$ (et donc $SR=0$).
